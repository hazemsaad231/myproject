...
const Add = () => {
    return (
        <div className="py-20 px-5 flex flex-col justify-center items-center ">
            ...
            {TheData.map((item) => (
                <div key={item.id} data-aos = "zoom-in" className="cursor-pointer  p-10 flex flex-col justify-center items-center bg-gray-100 rounded-4xl">
  ...
  <h1 className="title text-2xl md:text-3xl lg:text-4xl" data-aos = "zoom-in">إحصــائيـــاتـــنــا</h1>
    <hr className="w-10 sm:w-20 md:w-60 lg:w-96 h-2 bg-[#0c7f73] border-0 rounded" />
                ...
                {Statistics.map((item) => (
                <div key={item.id} data-aos = "zoom-in" className="cursor-pointer p-4 flex flex-col justify-center items-center rounded-4xl">
